@objc(PillarKeychainSwift) class PillarKeychainSwift : CDVPlugin {
  ...
  func set(command: CDVInvokedUrlCommand) {
    ...
    var result = ""
    var status: CDVCommandStatus = CDVCommandStatus_ERROR
    if(keychain.set(command.arguments[0] as! String, forKey: command.arguments[1] as! String)){
        if command.arguments.count == 3 {
        ...
        result = "Failed to set key \(command.arguments[1]) with value \(command.arguments[0])"
    }
